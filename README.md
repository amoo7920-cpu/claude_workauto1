...
1. [vercel.com](https://vercel.com)에서 이 저장소를 Import 합니다. 별도 빌드 설정은 필요 없습니다
   (정적 파일 + `api/*.py` 서버리스 함수를 Vercel이 자동으로 인식합니다).
...
서버리스 함수 실행 시간 제한 때문에 한 번에 생성 가능한 기사는 최대 3건으로
